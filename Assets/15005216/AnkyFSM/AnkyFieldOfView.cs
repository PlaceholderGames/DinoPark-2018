...
public class AnkyFieldOfView : MonoBehaviour {
    ...
    void FindVisibleTargets()
    {
        ...
        for (int i = 0; i < targetsInViewRadius.Length; i++)
        {
            ...
            if (target.gameObject.name != this.gameObject.name)
            {
                Vector3 dirToTarget = (target.position - transform.position);
                if (Vector3.Angle(transform.forward, dirToTarget) < stereoAngle / 2) //Can be seen in stereo
                {
                    distanceToTarget = Vector3.Distance(transform.position, target.position); // Only if we see it do we know how far away it is, due to stereo vision
                    //Debug.Log(distanceToTarget + target.gameObject.name);
    ...
    public bool EnemyNearAlert()
    {
        return EnemyIsNear;
    }

    public bool EnemyCloseAttack()
    {
        return EnemyIsClose;
    }
