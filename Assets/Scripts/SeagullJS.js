...
private var target : SeagullFlightPathJS;
...
private var otherSeagulls : SeagullJS[];
...
function Start ()
{
	...
	target = origin.GetComponent(SeagullFlightPathJS);
	transform.parent = null;
	transformComponent = transform;
	
	var tempSeagulls = new Component[0];
	if (transform.parent)
		tempSeagulls = transform.parent.GetComponentsInChildren(SeagullJS);
	objects = new Transform[tempSeagulls.length];
	otherSeagulls = new SeagullJS[tempSeagulls.length];
...
{ 
	...
	for(var i=0;i<objects.Length;i++)
	{
		...
		if(o != transformComponent)
		{
			...
			if (d < followRadius)
			{
				...
				f = d / followRadius;
				var otherSealgull : SeagullJS = otherSeagulls[i];
	...
	// Sounds
	if(SeagullSoundHeatJS.heat < Mathf.Pow(Random.value, 1 / soundFrequency / Time.deltaTime))
	{
		AudioSource.PlayClipAtPoint(sounds[Random.Range(0, sounds.length)], myPosition, 0.90);	
		SeagullSoundHeatJS.heat += (1 / soundFrequency) / 10;
	}
